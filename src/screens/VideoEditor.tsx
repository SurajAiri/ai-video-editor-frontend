...
const VideoEditor = () => {
  ...
  const isStatusBeforeProcessedInvalidSegment = (status: string): boolean => {
    ...
    const processedInvalidSegmentIndex = statusOrder.indexOf(ProjectStatus.PROCESSED_INVALID_SEGMENT);
  ...
  const checkProjectStatus = async () => {
    ...
    try {
      ...
      setStatusState({
        isLoading: false,
        error: error instanceof Error ? error.message : 'Error checking project status',
        data: null,
      });
    }
  };
  ...
  const isLoading = statusState.isLoading || transcriptLoading || invalidSegmentsLoading;
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 to-blue-100">
      <div className="container mx-auto p-6 space-y-6">
        <header className="bg-white rounded-xl shadow-lg p-6 border-l-4 border-blue-500">
          ...
          <p className="text-gray-500 mt-1">Split and refine your video segments</p>
        </header>
        ...
        {errors.length > 0 && (
          <div className="space-y-3">
            {errors.map((error, index) => (
              <Alert key={index} variant="destructive" className="bg-red-50 border border-red-200 text-red-800">
        ...
        {isLoading && (
          <div className="flex items-center justify-center p-16">
            <div className="flex flex-col items-center space-y-4">
              ...
              <p className="text-blue-600 font-medium">Loading your project data...</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100">
            <h2 className="text-xl font-semibold mb-4 text-gray-800 flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2 text-blue-500" viewBox="0 0 20 20" fill="currentColor">
                ...
                <path fillRule="evenodd" d="M10 4a1 1 0 100 2 1 1 0 000-2zm0 7a1 1 0 011 1v1a1 1 0 11-2 0v-1a1 1 0 011-1zm0-5a1 1 0 011 1v3a1 1 0 11-2 0V7a1 1 0 011-1z" clipRule="evenodd" />
              </svg>
              Project Information
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="bg-blue-50 p-4 rounded-lg">
                ...
                <p className="font-medium text-gray-800">{statusState.data.job_id}</p>
              </div>
              <div className="bg-blue-50 p-4 rounded-lg">
                <p className="text-sm text-gray-500">Status</p>
                <div className="flex items-center">
                  <span className={`inline-block h-2 w-2 rounded-full mr-2 ${
                    statusState.data.project_status === ProjectStatus.COMPLETED 
                      ? 'bg-green-500' 
                      : 'bg-yellow-500'
                  }`}></span>
                  <p className="font-medium text-gray-800">{statusState.data.project_status}</p>
                </div>
              </div>
              {statusState.data.data?.input_path && (
                <div className="bg-blue-50 p-4 rounded-lg md:col-span-2">
                  ...
                  <p className="font-medium text-gray-800 truncate">{statusState.data.data.input_path}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {!isLoading && !errors.length && transcript.length > 0 && invalidSegments.length > 0 && (
          <div className="bg-white rounded-xl shadow-md p-6 border border-blue-100">
